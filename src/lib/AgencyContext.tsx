...
export interface Agency {
  ...
  name: string;
  /**
   * THE AGENCY'S MARK - a two-letter monogram and a colour, so sixteen agencies are not
   * sixteen identical rows. Each half is OPTIONAL and absent means automatic: the monogram
   * follows the name, the colour is derived from the id. ABSENT MEANS NOBODY CHOSE, and one is derived from the id for display; the field
   ...
   */
  mark?: { monogram?: string; colour?: string } | null;
...
/**
 * ⚠ THE TENDER RULES NOW LIVE IN lib/tenderState.ts, AND ARE RE-EXPORTED HERE (AUDIT G93).
 *
 * They were moved, not copied, and not one line of them changed. They left because this file
 * imports `./firebase` and reads `localStorage` at module scope, so no test could reach them -
 * and the notifications panel needed the same predicates the screens use. A second copy is
 * what G92 was committed to prevent.
 *
 * Re-exported so every existing import of `isUnassigned`, `isIntakeOpen`, `currentTenderFor`
 * and `otherActiveAts` from this module keeps resolving. Both paths reach ONE definition.
 *
 * ⚠ IMPORTED AS WELL AS RE-EXPORTED, AND THAT IS NOT REDUNDANT. `export … from` forwards a
 * name to importers WITHOUT binding it in this module's own scope, and this file calls two of
 * them itself - `currentTenderFor` in the sign-in AT restore and `isUnassigned` in the oil
 * opening balance. Re-export alone left those three call sites referring to names that no
 * longer existed here. Nothing in the test suite could see it, because no test imports this
 * file - that is the very reason these predicates moved out. Only tsc caught it.
 */
import { isUnassigned, currentTenderFor, isIntakeOpen, otherActiveAts } from './tenderState';
export { isUnassigned, currentTenderFor, isIntakeOpen, otherActiveAts };
export type { IntakeGate } from './tenderState';
